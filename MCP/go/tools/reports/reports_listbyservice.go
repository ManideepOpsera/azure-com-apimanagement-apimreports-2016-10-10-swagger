...
func Reports_listbyserviceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		aggregationVal, ok := args["aggregation"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: aggregation"), nil
		}
		aggregation, ok := aggregationVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: aggregation"), nil
		}
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ApiManagement/service/%s/reports/%s%s", cfg.BaseURL, resourceGroupName, serviceName, aggregation, subscriptionId, queryString)
...
func CreateReports_listbyserviceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_reports_aggregation",
		mcp.WithDescription("Lists report records."),
		...
		mcp.WithString("aggregation", mcp.Required(), mcp.Description("Report aggregation.")),
		mcp.WithString("$filter", mcp.Description("The filter to apply on the operation.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Reports_listbyserviceHandler(cfg),
	}
}
